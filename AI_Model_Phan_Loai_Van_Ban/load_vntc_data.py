...
VNTC_DIR = os.path.join(os.path.dirname(__file__), "vntc_data", "Train")
...
def load_vntc_data() -> pd.DataFrame:
    # Lấy dữ liệu từ thư mục vntc_data/Train (được nén thành file rar khi đưa lên GitHub)
    data = []
...
def merge_with_existing_training_data():
    ...
    if os.path.exists(TRAINING_FILE):
        try:
            ...
            print(f"Không thể tải dữ liệu hiện tại: {e}")
            existing_data = None

    # Lấy dữ liệu từ vntc_data/Train
    new_data = load_vntc_data()
    print(f"\nDữ liệu mới từ vntc_data/Train: {len(new_data)} dòng")
...
if __name__ == "__main__":
    print("Bắt đầu load dữ liệu từ vntc_data/Train...\n")
